...
function Get-CommitMessage {
  ...
  if (-not [string]::IsNullOrWhiteSpace($script:Message)) { return $script:Message }
  if ($IsWindows) {
    try {
      Add-Type -AssemblyName Microsoft.VisualBasic -ErrorAction Stop
      return [Microsoft.VisualBasic.Interaction]::InputBox("Enter commit message for the homeassistant repo:", "Commit", $def)
    } catch { return Read-Host "Enter commit message (default: '$def')" }
  } else {
...
try {
  ...
  try { Git pull --ff-only origin $pubBranch } catch {
    Write-Error "⚠️ Pull failed or diverged. Resolve manually before running again."
    exit 1
  }
