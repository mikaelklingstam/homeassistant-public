...
| `1663398488822` | Set Exp device/end start time | `packages/nordpool_most_expensive_hours.yaml` (≈55) | No | Populates `input_datetime.device_exp_start_time` / `_end_time` from `sensor.expensive_hours_energy_tomorrow` once per day, giving downstream automations their “expensive window” bounds. |
